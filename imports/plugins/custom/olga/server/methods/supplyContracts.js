...
function initializeContract(userId, productId, quantity) {
  ...
  const supplyContractId = SupplyContracts.insert({
    userId: userId,
    productId: productId,
    quantity: quantity,
    orders: [],
...
function hasOpenOrders(productId) {
  const openOrders = Orders.find({ "productSupplies.productId": productId }).fetch();
...
function coverOrders(productId, quantity, supplyContractId) {
  ...
  while (contractQuantity > 0 && i < openOrders.length) {
    const openQuantity = getOpenQuantity(openOrders[i], productId);
    const supplyQuantity = openQuantity < contractQuantity ? openQuantity : contractQuantity;
    if (supplyQuantity > 0) {
...
function updateOpenQuantity(order, productId, supplyQuantity, supplyContractId) {
  const newSupplies = order.productSupplies;
...
export const methods = {
  ...
  "supplyContracts/create": function (productId, quantity) {
    ...
    const supplyContractId = initializeContract(userId, productId, quantity);
    if (!supplyContractId) {
      return null;
    }
    const coveredOrders = coverOrders(productId, quantity, supplyContractId);
